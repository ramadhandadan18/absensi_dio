...
class Riwayat_2 extends CI_Controller
{
    ...
    public function get_data_bulanan2($fill_divisi, $fill_tahun, $fill_bulan)
    {
        $_POST['fill_divisi'] = $fill_divisi;
        $_POST['fill_tahun'] = $fill_tahun;
        $_POST['fill_bulan'] = $fill_bulan;
        ...
        $mapping_riwayat = [];
        foreach ($Riwayat as $k => $v) {
            $mapping_riwayat[$v['no_pegawai']][$v['tanggal']][] = $v;
        }
        // echopre($mapping_riwayat);
        // echopre($Riwayat);
        $i = 0;
        if (!empty($Riwayat_pegawai)) {
            foreach ($Riwayat_pegawai as $k => $v) {
                ...
                $dt_final[$i]['late'] = '';
                $dt_final[$i]['overtime'] = '';
                // $dt_mapping_riwayat = isset($mapping_riwayat[$v['no_pegawai']][$v['tanggal']])
                if (isset($mapping_riwayat[$v['no_pegawai']][$v['tanggal']])) {
                    foreach ($mapping_riwayat[$v['no_pegawai']][$v['tanggal']] as $kk => $vv) {
                        $dt_final[$i]['nama_pegawai'] = strtoupper($vv['nama_pegawai']);
                        $checkout = true;
                        if ($vv['keterangan'] == 'CHECK IN') {
                            $checkout = true;
